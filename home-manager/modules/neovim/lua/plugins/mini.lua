return { "echasnovski/mini.nvim",
  ...
  config = function ()
    -- UI
    if _G.config_is_hm then require('mini.base16').setup({ palette = _G.stylixPalette }) end
      ...
      content = {
        active = function()
          ...
          local section_filestatus = function()
            local flags = {}
            if not vim.bo.modifiable then flags[#flags+1] = '󰏯'
            else
              if vim.bo.readonly then flags[#flags+1] = '󰌾' end
              if vim.bo.modified then flags[#flags+1] = '󰏫' end
            end
          ...
          local section_fileformat = function(args)
            return MiniStatusline.is_truncated(args.trunc_width) and '' or ({ dos = "", unix = "", mac = "" })[vim.bo.fileformat]
          end
          local section_filesize = function(args)
            local size = vim.fn.getfsize(vim.fn.getreg('%'))
            local size_str = size < 1024 and string.format('%dB', size) or size < 1048576 and string.format('%.2fKiB', size / 1024) or string.format('%.2fMiB', size / 1048576)
          ...
          return MiniStatusline.combine_groups({
            ...
            { hl = 'MiniStatuslineDevinfo', strings = {
              MiniStatusline.section_diagnostics({ trunc_width = small, icon = "", signs = { ERROR = '', WARN = '', INFO = '', HINT = '' } }),
    ...
    require('mini.clue').setup({
      ...
      clues = (function() local c = require('mini.clue'); return {
