{ config, ... }: {
  ...
  in {
    ...
    defaultOptions = [
      "--ansi"
      "--keep-right" "--info=inline-right"
      "--marker=◆" "--ellipsis=…" "--header='${basicHeader}'" "--prompt='fzf'"
      "--bind='${basicBinds}'"
    ];
    fileWidgetCommand = "${config.programs.fzf.defaultCommand} -t file";
    fileWidgetOptions = config.programs.fzf.defaultOptions ++ [
      "--header='${fdHeader}'" "--prompt=' '" "--preview-window=border-sharp"
    ...
    changeDirWidgetOptions = config.programs.fzf.defaultOptions ++ [
      "--header='${fdHeader}'" "--prompt=' '" "--preview-window=border-double"
