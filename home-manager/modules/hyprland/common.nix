{ flake, lib, config, pkgs, ... }: rec {
  ...
  volume-command = dev: amt:
    "${pkgs.wireplumber}/bin/wpctl set-volume ${dev} ${fmt-adj amt} ${if amt > 0 then "&& wpctl set-mute ${dev} 0" else ""} && pw-cat -p ${flake.inputs.ocean-sound-theme}/ocean/stereo/audio-volume-change.oga";
