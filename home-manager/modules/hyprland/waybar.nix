...
in {
  programs.waybar = {
    ...
    settings = let
      ...
      generate-formats = name-pre: fmt: lib.mapAttrs' (
        val: sym: { name = "${name-pre}${val}"; value = builtins.replaceStrings ["{}"] [sym] fmt; }
      );
    in [rec {
      position = "top";
      modules-left = [
      ...
      cpu = { states = generate-states braille-scale; }
      ...
      // (generate-formats "format-" "{}" braille-scale);
      disk = { states = generate-states braille-scale; }
      // (generate-formats "format-" "{}" braille-scale);
      temperature = {
